...
Requires: xgb_models.pkl, optimal_thresholds.json, spw_dict.json,
          ensemble_results.csv, feature_importances.csv,
          feature_category_summary.csv, optimal_thresholds.csv
"""
...
# ── FEAT_COLS embedded (reconstructed from feature_importances.csv + model indices) ─────
# This is the exact feat_cols order the training pipeline used.
# Positions with '__zero__N' are VT-filtered-in but zero-gain bits (safe to leave 0).
@st.cache_data
def load_feat_cols():
    p = os.path.join(DIR, 'feat_cols.json')
    if os.path.exists(p):
        with open(p) as f:
            return json.load(f)
    # Fallback: return None (app will warn)
    return None

@st.cache_resource(show_spinner="Loading models…")
def load_models():
    m = {}
    xp = os.path.join(DIR, 'xgb_models.pkl')
    if os.path.exists(xp):
        with open(xp,'rb') as f: m['xgb'] = pickle.load(f)
    ep = os.path.join(DIR, 'ensemble_models.pkl')
    if os.path.exists(ep):
        with open(ep,'rb') as f: m['ensemble'] = pickle.load(f)
    bp = os.path.join(DIR, 'brf_models.pkl')
    if os.path.exists(bp):
        with open(bp,'rb') as f: m['brf'] = pickle.load(f)
    cp = os.path.join(DIR, 'cat_models.pkl')
    if os.path.exists(cp):
        with open(cp,'rb') as f: m['cat'] = pickle.load(f)
    return m

@st.cache_data
def load_meta():
    def rj(n): p=os.path.join(DIR,n); return json.load(open(p)) if os.path.exists(p) else {}
    def rc(n): p=os.path.join(DIR,n); return pd.read_csv(p) if os.path.exists(p) else pd.DataFrame()
...
if page == "🔮  Predict":
    ...
    if feat_cols is None:
        st.markdown('<div class="err">⚠️ <b>feat_cols.json not found.</b> Place it in the same directory as app_final.py.</div>',unsafe_allow_html=True)
    ...
    if load_btn and selected_example != "— select a compound —":
        ...
        if mol_ex and models.get('xgb') and feat_cols:
            ...
            st.markdown('<div class="err">⚠️ Models not loaded — place xgb_models.pkl in the app directory.</div>',
                        unsafe_allow_html=True)
    ...
    if go:
        if not smi_in.strip():
            ...
            if mol_try is None:
                ...
                st.markdown('<div class="err">⚠️ Models not loaded. Check xgb_models.pkl and feat_cols.json.</div>', unsafe_allow_html=True)
            else:
...
# ══════════════════════════════════════════════════════════════════
    ...
    with ta:
        ...
        roc=os.path.join(DIR,'roc_curves_ensemble.png')
...
# ══════════════════════════════════════════════════════════════════
    ...
    fip=os.path.join(DIR,'feature_importance_analysis.png')
    if os.path.exists(fip): st.image(fip,use_container_width=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with tb:
        ...
        if mc and models.get('xgb') and feat_cols:
            ...
            st.info("Place xgb_models.pkl and feat_cols.json in the app directory to enable predictions.")