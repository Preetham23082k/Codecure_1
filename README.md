...
2. [Dataset](#dataset)
3. [Evaluation Metrics — ROC-AUC, Recall & PR-AUC](#evaluation-metrics--roc-auc-recall--pr-auc)
...
## Evaluation Metrics — ROC-AUC, Recall & PR-AUC

This pipeline evaluates model performance using **three complementary metrics**, each capturing a distinct and critical aspect of toxicity prediction quality. No single metric is sufficient in isolation on a dataset as imbalanced and structurally complex as Tox21.

---

### 1. ROC-AUC (Primary Benchmark Metric)

ROC-AUC (Area Under the Receiver Operating Characteristic Curve) is the primary metric for benchmarking and model selection, for several compounding reasons:

**Threshold independence.** The optimal decision threshold for flagging a compound as toxic varies per endpoint and per downstream use case. ROC-AUC evaluates the entire score distribution rather than fixing a threshold, which is why threshold optimisation is handled separately in Stage 4.

**Imbalance robustness for ranking.** ROC-AUC measures the model's ability to rank toxic compounds above non-toxic ones, independent of class distribution. A random classifier always scores 0.5 regardless of imbalance; a perfect classifier scores 1.0. This makes it a consistent, comparable metric across all 12 endpoints despite their differing positive rates (3%–25%).

**Standard for Tox21 benchmarks.** The original Tox21 challenge and all major subsequent publications (DeepChem, ChemProp, AttentiveFP) use mean ROC-AUC across the 12 endpoints as the primary benchmark metric, enabling direct comparison with the literature.

**Interpretability of ensemble improvement.** The delta in ROC-AUC between models directly quantifies ranking improvement — our 3-tier ensemble achieves +0.014 mean ROC-AUC over the XGBoost baseline.

---

### 2. Recall (Sensitivity) — Safety-Critical Metric

Recall (true positive rate) measures what fraction of actually toxic compounds the model correctly identifies. In drug safety prediction, **recall is the most clinically consequential metric** because:

- A **false negative** (predicting a toxic compound as safe) is far more dangerous than a false positive. Missing a toxic compound in early screening can lead to costly late-stage failures or, worse, patient harm.
- At the default threshold of 0.5, recall is only 35–45% across most endpoints — meaning over half of toxic compounds are missed.
- Our F1-optimal per-endpoint threshold tuning (Stage 4) explicitly targets **Recall ≥ 0.65 with Precision ≥ 0.35**, lifting weighted recall by **60%** over the default threshold.
- On NR-PPAR-gamma specifically, recall improves from **0.285 → 0.737 (+158%)** after SMOTE and cost reweighting.

Recall is reported per endpoint alongside the optimised threshold, making it the primary operational metric for any deployment scenario prioritising safety.

---

### 3. PR-AUC (Precision-Recall AUC) — Honest Imbalance Metric

PR-AUC (Area Under the Precision-Recall Curve) is the most honest metric for evaluating model quality on severely imbalanced datasets:

**Why ROC-AUC alone can be misleading.** On an endpoint with 4.3% positive rate (NR-AR), a model can achieve high ROC-AUC by correctly classifying the massive number of true negatives — the easy majority class. PR-AUC only rewards the model when it correctly identifies the rare toxic class, making it a far stricter and more informative measure of minority-class performance.

**Lift over random baseline.** A random classifier on NR-AR achieves PR-AUC = 0.04 (equal to the prevalence rate of 4.3%). Our model achieves **PR-AUC = 0.53 on NR-AR — a 12.5× lift**. Mean PR-AUC lift across all 12 endpoints is **9.5×**, demonstrating consistent and substantial improvement over chance across the full benchmark.

**Absolute PR-AUC understates quality.** A PR-AUC of 0.53 on a 4.3% prevalence endpoint represents exceptional discrimination — it should not be compared against PR-AUC scores from balanced datasets. The lift over random baseline is always the correct frame of reference.

PR-AUC is used during model development to guide CatBoost training on SPECIAL tier endpoints (NR-ER uses `aucpr` as the eval objective), where the class imbalance is most severe and ROC-AUC alone would mask poor minority-class performance.

---

### Summary

| Metric | Role | Key Result |
|--------|------|------------|
| **ROC-AUC** | Benchmark & model selection | +0.014 ensemble gain over XGBoost baseline |
| **Recall** | Safety-critical operational metric | +60% weighted recall via threshold tuning; +158% on NR-PPAR-gamma |
| **PR-AUC** | Honest imbalance evaluation | 9.5× mean lift over random baseline; 12.5× on NR-AR |

All three metrics are computed per endpoint and reported in `results/ensemble_results.csv` and `results/optimal_thresholds.csv`.
